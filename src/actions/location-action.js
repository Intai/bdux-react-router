...
const createBinder = (() => {
  ...
  return sink => (
    // if creating binder to reactivate the bacon stream.
    (count++ > 0)
      // skip the current location.
      ? skipFirst(pushLocation(sink))
      // otherwise sink the current location.
      : pushLocation(sink)
  )
})()
...
export const listen = onceThenNull(() => (
  Bacon.fromBinder((sink) => {
    return getHistory().listen(createBinder(sink))
  })
